...
namespace LocAlgorithm
{
    public class LocAlgV4
    {
        ...
        #region 构造器
        public LocAlgV4(int e)
        {
            //┌-------基本信息初始化--------
            this.environmentId = e;
            this.type = LocContract.TagType.WIFI;
            ...
            {
                enableFlag = false;
                Log.AddLogEntry(LogEntryLevel.ERROR, "LocAlgV4:不存在环境Id" + this.environmentId.ToString());
            }
        ...
        /// <returns></returns>
        public SingleLocResult Loc(List<APRssiPair> apRssiPairList, DateTime currentTime, bool debugEnable, out bool IsDisappered)
        {
            if (enableFlag == false)
            {
                //已经无效，因此不能认为是消失或远离定位区域
                IsDisappered = false;
                return null;
            }
            ...
            try
            {
                ...
                if (IsLocAvalibe == false)
                {
                    ...
                    #endregion
                    if (failedMeetLocConditionTimes >= 6)
                    {
                        //只有连续6次不满足定位条件才会认为其消失
                        IsDisappered = true;
                ...
                if (avalibleAPRssiPairList.Count < Parameters.LOC_CONDITION_COUNT)
                {
                    ...
                    Log.AddLogEntry(LogEntryLevel.ERROR, "执行定位条件检查后avalibleAPRssiPairList.Cout<2!");
                }
                else
                {
                ...
                #endregion
                //┗━━━━━━━━━━━━━━━━━━━
                if (maxRssi > InnerClasses.Parameters.CHANGE_POSITION_RSSI && divValue > InnerClasses.Parameters.CHANGE_POSITION_DVALUE)
                {
                    //AP快速胜出
                    #region AP快速胜出
            ...
            {
                Log.AddLogEntry(LogEntryLevel.ERROR, "LocAlgV4.Loc执行错误：" + ex.Message);
                IsDisappered = false;
                return null;
            }
        }
